...
class AnalyseScreen(ModalScreen):
    ...
    def __init__(
        ...
        super().__init__(name, id, classes)
        self.target_path = target_path
        self._analysis_output_content: str | None = (
            None  # To store output before saving/displaying
        )
    ...
    def compose(self) -> ComposeResult:
        with Container(id="analyse_options_container"):
            ...
            yield Input(
                placeholder="e.g., Initial security audit", id="description_input"
            )
    ...
    def display_current_page(self) -> None:
        ...
        if not self.results:
            results_container.mount(
                Static("No results found for this query.", id="no_results_message")
            )
...
class QueryScreen(ModalScreen):  # Could be a regular Screen too
    ...
    def compose(self) -> ComposeResult:
        with Container(id="query_options_container"):
            ...
            yield Input(
                placeholder="e.g., 'vulnerability', 'libc', 'CVE-2023-...",
    ...
    def _generate_dummy_results(self, search_term: str, count: int) -> list[dict]:
        ...
        for i in range(count):
            result = {
                ...
                "details": f"Detailed information for item {i+1}. This could be a very long string.",
                "tags": random.sample(
                    ["tag-a", "tag-b", "tag-c", "tag-d", "tag-e"], random.randint(1, 3)
                ),
            }
